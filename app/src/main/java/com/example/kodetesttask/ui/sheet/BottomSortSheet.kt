...
class BottomSortSheet : BottomSheetDialogFragment() {
	...
	override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
		...
		binding.radioButton.setOnClickListener {
			(parentFragment as HomeFragment).setSortType(UsersListFragment.SortType.ALPHABET)
		}
		binding.radioButton2.setOnClickListener {
			(parentFragment as HomeFragment).setSortType(UsersListFragment.SortType.DATE)
		}
	}

	private fun checkSorts() {
		if (
			(parentFragment as HomeFragment).getCurrentViewPagerItemFragment()?.getSort() ==
			UsersListFragment.SortType.ALPHABET
		)
